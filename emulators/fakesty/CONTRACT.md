# Fakesty — HTTP contract (Etsy Open API v3–shaped emulator)

CrossListr should implement against **this document** for Fakesty, not against live Etsy without reading Etsy’s own reference. Real Etsy requires API keys, OAuth approval, and stricter payload rules; Fakesty is a **dev shortcut** with **Etsy-flavored paths and headers**.
...
Containers on the shared integration network typically call `http://fakesty-backend:8080`.
...
| `FAKESTY_UPLOAD_DIR` | Writable directory for listing image files (default `/tmp/fakesty-upload`). |
| `FAKESTY_PUBLIC_ORIGIN` | Base URL embedded in `url_fullxfull` JSON (e.g. `http://127.0.0.1:14382`). |
| `FAKESTY_API_KEY` | When set, requests must send **`X-Api-Key`** with this exact value (mirrors Etsy’s **`x-api-key`** pattern). |
| `FAKESTY_ACCESS_TOKEN` | When set, requests must send **`Authorization: Bearer`** with this token. When both this and `FAKESTY_API_KEY` are unset, auth is **off** (dev-only). |
...
Production Etsy returns more image metadata (dims, hues, etc.); Fakesty only guarantees fields needed for **CrossListr download / re-host** practice.
