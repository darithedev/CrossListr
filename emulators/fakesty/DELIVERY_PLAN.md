# Fakesty — delivery plan (Etsy-shaped emulator)

## Purpose

**Fakesty** is a local **Etsy Open API–shaped** emulator so **CrossListr** can practice **listing** and **shop** flows with Etsy-like auth and payloads—without Etsy API approval for early development.
...
| Layer | Choice |
|-------|--------|
| API | **Go** in **`emulators/fakesty/backend/`** |
| UI | **React** in **`emulators/fakesty/frontend/`** |
| Data | **Postgres**, **internal-only** |
| Migrations | **golang-migrate**, **`emulators/fakesty/db/migrations/`**, **`emulators/fakesty/db/Dockerfile`** |
...
- **`emulators/fakesty/compose.yaml`**: Postgres → migrate → backend → frontend; **unique ports** vs CrossListr, FakeBay, and Fakify.
- Network e.g. **`fakesty`** + shared **`integration`** for CrossListr.
- Postgres **not** published to host.

## Git workflow

Chained feature branches; suggested prefixes: `fs/00-scaffold`, `fs/01-health`, `fs/02-etsy-oauth`, `fs/03-listings-crud`, `fs/04-purchase-stub`, `fs/05-fakesty-ui`, `fs/06-crosslistr-stub`.
...
| **06** | CrossListr stub | Env-driven **Fakesty** base URL + auth; multi-platform tests with FakeBay + Fakify. |
...
| Topic | Decision |
|-------|----------|
| **Repository** | **`emulators/fakesty/`** in the same monorepo. |
...
*Fakesty — Etsy clone emulator for CrossListr multi-platform field mapping.*
