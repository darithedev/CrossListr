...
export default function ListingsPage() {
  ...
  return (
    <main className="wrap">
      <h1>Your listings</h1>
      <p className="lead">
        Signed in as <strong>{session.email}</strong>. Rows are scoped to your emulator shop via{' '}
        <code>default_shop_id</code>.
      </p>
      {loadErr && <p className="err">{loadErr}</p>}
      <section className="listings-stack">
        {listings.length === 0 ? (
          <div className="panel">
            <p className="muted empty-hint">No listings for this shop yet.</p>
          </div>
        ) : (
          listings.map((l) => (
