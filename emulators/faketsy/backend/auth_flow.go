...
func validateAuthzQuery(q url.Values, cfg oauthConfig) (*authzParams, string, string, string) {
	...
	method := normalizePKCEMethod(q.Get("code_challenge_method"))
	...
	if challenge == "" {
		return nil, redirectURI, "invalid_request", "code_challenge is required (PKCE)"
	}
	if method != "S256" {
		return nil, redirectURI, "invalid_request", "code_challenge_method must be S256"
	}
...
<html lang="en"><head><meta charset="utf-8"><title>Faketsy — Sign in</title>
...
<p class="lead">Etsy-pattern emulator — next you’ll approve access for the requesting app.</p>
...
<html lang="en"><head><meta charset="utf-8"><title>Faketsy — Approve access</title>
<meta name="viewport" content="width=device-width,initial-scale=1">
<style>
...
<div class="box">
<p><strong>{{.AppName}}</strong> wants to connect using Etsy-shaped OAuth (PKCE).</p>
<p class="muted">If you approve, {{.AppName}} can call the API on your behalf (dev emulator).</p>
...
func handleLoginGet(w http.ResponseWriter, r *http.Request) {
	...
	_ = loginTpl.Execute(w, loginPage{ReturnTo: target, EmailDefault: "demo@faketsy.local"})
}
...
func handleOAuthConnect(w http.ResponseWriter, r *http.Request, cfg oauthConfig, sm *sessionManager) {
...
<html lang="en"><head><meta charset="utf-8"><title>Authorize — Faketsy</title></head>
<body><p>Open this URL from your app with OAuth query params (<code>client_id</code>, <code>redirect_uri</code>, PKCE <code>code_challenge</code>, …), or <a href="/login">sign in</a> first via the demo.</p>
...
func handleOAuthConsentPost(w http.ResponseWriter, r *http.Request, cfg oauthConfig, store *oauthStore, sm *sessionManager) {
	if err := r.ParseForm(); err != nil {
		http.Error(w, "bad form", http.StatusBadRequest)
		return
	}
	...
	sub := strconv.FormatInt(sess.userID, 10)
	code := store.issueAuthCode(p.ClientID, p.RedirectURI, sub, cfg.AuthCodeTTL, p.CodeChallenge, p.CodeChallengeMethod)
